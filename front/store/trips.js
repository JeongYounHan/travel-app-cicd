...
export const actions = {
  FETCH_CITYLIST({ commit }, payload) {
    ...
    this.$axios
      .get(`${BACK_URL}/cities?search=${keyword}`)
      .then(res => {
  ...
  FETCH_PLACELIST({ commit }, payload) {
    ...
    this.$axios
      .get(`${BACK_URL}/places?city=${cityId}`)
      .then(res => {
  ...
  FETCH_SCHEDULELIST({ commit, rootState }, payload) {
    ...
    this.$axios
      .get(`${BACK_URL}/schedules?trip=${payload.trip}`, config)
      .then(res => {
  ...
  ADD_SCHEDULE({ dispatch, rootState }, payload) {
    ...
    this.$axios
      ...
      .then(res => {
        ...
        dispatch('FETCH_DAYSCHEDULE', {
          trip: res.data.trip.id,
          day: payload.day
        })
      })
      .catch(err => {
  ...
  DELETE_SCHEDULE({ dispatch, rootState }, payload) {
    ...
    this.$axios
      .delete(`${BACK_URL}/schedules/${payload.id}`, {}, config)
      .then(() => {
  ...
  FETCH_DAYSCHEDULE({ commit, rootState }, payload) {
    ...
    return this.$axios
      .get(`${BACK_URL}/schedules?trip=${payload.trip}&day=${day}`, config)
      .then(res => {
  ...
  async FETCH_TRIP({ commit, rootState }, payload) {
    const config = {
      headers: {
        Authorization: 'Token ' + rootState.users.token
      }
    }
    const res = await this.$axios.get(`${BACK_URL}/trips/${payload}`, config)
    commit('SELECT_TRIP', res.data)
    return res.data
  }
}
