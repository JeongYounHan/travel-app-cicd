...
export default {
  ...
  computed: {
    ...
    daySchedule() {
      let temp = this.$store.state.trips.scheduleList.filter(
        v => v.day == this.day
      )
      // order 순서별로 정렬
  ...
  methods: {
    ...
    setDragAndDrop() {
      ...
      this.$dragula([from], {
        ...
        // 옮긴 객체 id
        const cur = parseInt(
          el.getElementsByClassName('schedule__div')[0].dataset['scheduleId']
        )
        ...
        const targetSchedule = {
          id: cur,
          order: 0,
          day: el.getElementsByClassName('schedule__div')[0].dataset[
            'scheduleDay'
          ]
        }
        ...
        if (!prev && next) {
          ...
          targetSchedule.order =
            (parseFloat(prev.dataset['scheduleOrder']) +
              parseFloat(next.dataset['scheduleOrder'])) /
            2
        }
